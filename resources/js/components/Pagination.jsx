...
const Pagination = ({ currentPage, lastPage, onPageChange }) => {
  const generatePageNumbers = () => {
    ...
    } else if (currentPage >= lastPage - Math.floor(totalPageNumbersToShow / 2)) {
      // Show the last n page numbers
      return Array.from({ length: totalPageNumbers }, (_, i) => lastPage - totalPageNumbers + i + 1);
    } else {
      // Show a range of page numbers centered around the current page
      return Array.from({ length: totalPageNumbers }, (_, i) => currentPage - Math.floor(totalPageNumbers / 2) + i);
    }
  };
